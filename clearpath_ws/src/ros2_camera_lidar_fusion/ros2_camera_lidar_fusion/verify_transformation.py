...
class LidarCameraProjectionNode(Node):
    def __init__(self):
        ...
        # Subscribers with fallback logic
    ...
    def timer_callback(self):
        if self.latest_image and self.latest_lidar:
            self.sync_callback(self.latest_image, self.latest_lidar)
    ...
    def sync_callback(self, image_msg: Image, lidar_msg: PointCloud2):
        ...
        self.get_logger().info(f"✅ Projected {count}/{len(image_points)} points.")
        ...
        cv2.imshow("Projected LiDAR Points", cv_image)
        cv2.waitKey(1)
